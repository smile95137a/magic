import godCaishen from '@/assets/image/god_caishen.png';
import godGuangong from '@/assets/image/god_guangong.png';
import godWenchang from '@/assets/image/god_wenchang.png';
import godYuelao from '@/assets/image/god_yuelao.png';
import godZhongkui from '@/assets/image/god_zhongkui.png';

import godCaishenG from '@/assets/image/god_caishen_g.png';
import godGuangongG from '@/assets/image/god_guangong_g.png';
import godWenchangG from '@/assets/image/god_wenchang_g.png';
import godYuelaoG from '@/assets/image/god_yuelao_g.png';
import godZhongkuiG from '@/assets/image/god_zhongkui_g.png';

const godImages = {
  caishen: {
    label: '財神',
    default: godCaishen,
    highlighted: godCaishenG,
    description: `在下玄壇真君 趙公明,統帥「招寶天尊蕭昇」、「納珍天尊曹寶」、「招財使者陳九公」、「利市仙官姚少司」四位神仙專司迎祥納福、商賈買賣。祂們分別負責「招寶」、「納珍」、「招財」 和「利市」等財富的神,所以稱在下為財神。
      《封神演義》金龍如意正一龍虎玄壇真君"
      <br><br>
多年後有本書叫《封神演義》,裡面說著姜子牙並沒有把在下封為財神,只封為「金龍如意正一龍虎玄壇真君」,簡稱「玄壇真君」,但在下與四位部屬負責各路財富,後來大家將在下們合稱為五路財神,希望我們可以為人們帶來東方、南方、西方、北方與中央五個方位的財運。
"台灣農曆正月特有的廟宇活動「炸寒單」"
在台灣,在下被稱「寒單爺」,人們傳說在下懼冷,所以每年農曆正月,有些廟宇就會舉辦「炸寒單」的活動:請人赤裸半身,手持樹枝,扮演在下的模樣。人們便以鞭炮丟擲扮演在下的人,以讓在下可以感受溫暖,可以保佑大家。`,
  },
  guangong: {
    label: '關公',
    default: godGuangong,
    highlighted: godGuangongG,
    description: `我姓關名羽,字雲長,乃三國時期蜀國大將。自大哥-劉備於鄉里聚眾起兵開始我就追隨他,馳騁沙場,盡職盡忠。我為人「義」字為先,重情重義,即使身在曹營,卻仍然誓死要追隨我大哥,「雲長真義士也」就是曹操對我的評價。我死後,我的形象逐漸被後人神化・古往今來一直是民間祭祀的對象,被尊為「關公」。
    <br><br>
     "不止講義氣,經商會計也在行"
<br><br>
人們信奉、參拜我的理由有三:一是我生前精於計算,被後世公認為會計專才,也被稱為「商業神」;二是因為我做人、做事重義氣和信用;三是人們希望可以像我一樣,即使失敗了,也能東山再起,取得最後的勝利。`,
  },
  wenchang: {
    label: '文昌',
    default: godWenchang,
    highlighted: godWenchangG,
    description: `"掌管人間功名利祿的文運之神"
在下姓張名亞子,我的名字大家可能不是很熟悉,但是只要每逢各種大小考試,善男信女都會來我這裡祈求添賜智慧、金榜題名,考試順利。是的,沒錯!本仙就是掌管人間功名利祿的文運之神,也就是赫赫有名的文昌帝君,又稱「梓潼帝君」、「文昌君」。
"元仁宗封為:輔文開化文昌司祿宏仁帝君"
<br><br>
說到我的事蹟,那可要先從東晉時期說起,當時,我是一個對母親非常孝順的人,後來,不幸戰死在沙場,當地人幫我立了一座廟, 讚頌我的英勇事蹟。之後則成為梓潼的地方神,叫做梓潼神。
因為科舉制度盛行,考生要進京趕考之前,會請在地家鄉神明來庇佑,於是在我庇佑之下,大家紛紛覺得靈驗,漸漸的我就越來有知名度嘍!也因為這樣,玉皇大帝讓我掌管文昌府和人間的功名利祿利祿。那更之後呢?我又跟文昌星逐漸合而為一,神力也就更加強大。到了元代,元仁宗封我為「輔文開化文昌司祿宏仁帝君」,這麼長是不是不好念?沒關係,大家可以簡單的稱呼我為文昌帝君就可以了。`,
  },
  yuelao: {
    label: '月老',
    default: godYuelao,
    highlighted: godYuelaoG,
    description: `"俺是月下老人,掌管男女婚姻"
俺是月下老人,有人稱俺月老、月老爺或月老公,是掌管男女婚姻的神明。特色是一嘴雪白的鬍鬚,雙頰泛紅光;左手拿著姻緣簿, 右手拄著拐杖。
<br><br>
《唐·李復言·續幽怪錄‧定婚店》記載著,有一次在宋城的客棧裡, 當俺正在月光下翻閱姻緣簿的時候遇到一個少年叫韋固,那少年問俺這本書是什麼?俺回答他說:『這本書叫「鴛鴦譜」是全天下人的婚姻登記。你瞧瞧路邊那個賣菜老嫗的女兒,現在三歲,就是你十四年後的妻子。』,那少年嫌那女娃兒不漂亮,找人刺傷那個小女孩。結果,十幾年後,那位少年結婚了,看到妻子眉上有道疤痕,發現就是當年被他找人刺傷的小女娃。後來,宋城縣宰知道這個故事之後,把那少年遇到俺的客棧定名叫「定婚店」。而俺就成了中國人心目中,對愛情期待的善男信女們信奉最忠誠的神明了。`,
  },
  zhongkui: {
    label: '鐘馗',
    default: godZhongkui,
    highlighted: godZhongkuiG,
    description: `俺就是大名鼎鼎,翊聖雷霆,驅魔辟邪,文武雙全,剛正不阿,肝膽相照的「鎮宅賜福帝君」。簡稱「鎮宅真君」、「驅魔真君」, 專門負責打鬼、驅除邪祟。雖然俺長得不夠帥,但也有幾分個性另加霸氣外露。
    <br><br>
    "貼鍾馗門神,驅除邪祟一把罩"
    <br><br>
    斬妖除魔本是在下的職責,憑借我鍾馗的本事,打打幾個小鬼根本就是舉手之勞,不足掛齒。可大家總喜歡把我捧得很高,甚至有人說我是「萬應之神」,可以要福得福,要財得財,有求必應。總之,把我說得神乎其神。也正因為我本領這麼高,凡間喜歡把我的個人寫真貼在自家大門上。`,
  },
};

export default godImages;
